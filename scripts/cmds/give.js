/**
 * @author Shade & AI
 * @title Transfert d'argent Canvas Premium (Émeraude)
 * @name give
 * @class give
 * @version 1.3.0
 * @description Donne de l'argent de portefeuille à un autre utilisateur avec le design émeraude officiel de la Balance Card.
...
module.exports = {
  config: {  
    ...
    role: 0, 
    author: "Shade & AI",  
    description: "Donne de l'argent via tag ou réponse avec interface Émeraude identique à la balance card",  
    category: "economy",  
    guide: {
  ...
  onStart: async function ({ api, event, args, usersData }) {  
    ...
    // === GÉNÉRATION DE L'INTERFACE CANVAS (STYLE BALANCE CARD ÉMERAUDE) ===  
    const canvas = createCanvas(850, 350);  
    const ctx = canvas.getContext("2d");  
    
    // Fond Matrix Dégradé (Identique à BalanceC)
    ...
    // Double bordure fine de sécurité (Identique à BalanceC)
    ...
    // Ligne centrale de liaison Émraude
    ctx.save();
    ctx.shadowColor = "#22c55e";
    ctx.shadowBlur = 10;
    ...
    // Montant Vert Émeraude (Compacté avec formatMoney)
    ctx.save();
