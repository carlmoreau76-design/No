const SHOP_ITEMS = {  
  ...
  bijoux: [  
    ...
    { id: "b14", name: "Chaîne Or Blanc 24k", price: 85000, desc: "Pureté maximale" },  
...
module.exports = {  
  config: {    
    ...
    role: 0,    
    category: "economy",    
    shortDescription: "Système de magasin à 10 rayons avec inventaire persistant corrigé",    
    guide: {      
  ...
  onStart: async function ({ message, event, args, usersData }) {    
    ...
    // Détermination propre de l'action principale via les arguments ou la commande d'appel
    const threadPrefix = global.utils.getPrefix(event.threadID);
    const commandText = event.body.slice(threadPrefix.length).trim().split(/\s+/)[0].toLowerCase();
    
    let action = args[0] ? args[0].toLowerCase() : null;
    
    // Si l'utilisateur tape directement un alias comme "possede", on force l'action
    if (["possede", "possède"].includes(commandText)) {
      action = "possede";
    }

    // Récupération sécurisée du profil utilisateur complet
    const userData = await usersData.get(senderID);    
    if (!userData) return message.reply("❌ **[ERREUR]** Impossible de charger votre profil économique.");    

    // Initialisation stricte de l'objet profond de persistance
    if (!userData.customData) userData.customData = {};    
    if (!userData.customData.shop) userData.customData.shop = {};    
        
    // Mapping préventif pour s'assurer de l'existence des catégories
    Object.keys(SHOP_ITEMS).forEach(cat => {      
      if (!userData.customData.shop[cat] || !Array.isArray(userData.customData.shop[cat])) {        
    ...
    // ==========================================    
    if (action === "possede" || action === "possède") {      
    ...
    if (action === "buy") {      
      ...
      // Utilisation du champ natif de GoatBot ou fallback sur userData.money
      const currentMoney = await usersData.get(senderID, "money") || 0;

      if (currentMoney < foundItem.price) {        
        return message.reply(`🟥 **[FONDS INSUFFISANTS]** Solde requis : **${foundItem.price.toLocaleString()}$**. Votre solde : **${currentMoney.toLocaleString()}$**.`);      
      }      

      // Ajout à la copie de l'inventaire courant
      ...
      // Assignation de l'objet profond mis à jour
      userData.customData.shop = inv;      

      try {
        // Soustraction de l'argent via le setter dédié de la base
        await usersData.set(senderID, currentMoney - foundItem.price, "money");
        
        // Sauvegarde explicite et forcée du bloc customData pour éliminer la non-persistance
        await usersData.set(senderID, userData.customData, "customData");
        
        return message.reply(`🎉 **TRANSACTION APPROUVÉE !**\n\nVous venez d'acquérir : **${foundItem.name}**\n📉 Facturation : -**${foundItem.price.toLocaleString()}$**\n💳 Nouveau solde : **${(currentMoney - foundItem.price).toLocaleString()}$**.\n\n💡 Tapez \`shop possede\` pour ouvrir votre inventaire.`);    
      } catch (err) {
        return message.reply(`❌ **[ERREUR SAUVEGARDE]** Échec de la synchronisation de l'achat : ${err.message}`);
      }
    }    

    // Menu d'accueil par défaut si aucune action ou argument n'est fourni
    const userMoney = await usersData.get(senderID, "money") || 0;
    let defaultMenu = `Bienvenue sur le terminal du marché noir.\n💳 Votre solde : ** ${userMoney.toLocaleString()}$**\n\n🏬 **Secteurs d'approvisionnement :**\n`;    
