...
async function createBalanceCard(userData, userID, balance, avatarImage) {
    const width = 950;
    const height = 520;
    ...
    const formattedAmount = `${CURRENCY_SYMBOL}${formatMoney(balance)}`;
    ctx.save();
    for (let i = 12; i > 0; i--) {
        ctx.fillStyle = `rgba(34, 197, 94, ${0.015 * i})`;
        ctx.font = `bold ${68 + i * 0.5}px "NotoSans", "BeVietnamPro", sans-serif`;
        ctx.fillText(formattedAmount, 48 + (12 - i) * 0.2, 248 + (12 - i) * 0.2);
    }
    ctx.restore();
    ...
    ctx.fillText(formattedAmount, 50, 250);
    ...
    if (avatarImage) {
        ...
        ctx.closePath();
        ctx.clip();
        ctx.drawImage(avatarImage, picX, picY, picSize, picSize);
        ctx.restore();
    } else {
...
module.exports = {
    config: {
        name: "balance",
        aliases: ["bal", "wallet", "mybalance", "$"],
        version: "2.2.0",
        author: "Neoaz ゐ × Gemini",
        countDown: 10,
        role: 0,
    ...
    onStart: async function({ message, event, usersData, args }) {
        try {
            ...
            let avatarImage;
            try {
                // Utilisation de la méthode interne stable de GoatBot pour récupérer l'avatar
                avatarImage = await loadImage(await usersData.getAvatarUrl(targetID));
            } catch (err) {
                // Image par défaut si le chargement échoue
                avatarImage = await loadImage("https://i.imgur.com/I3VsBEt.png").catch(() => null);
            }
            
            const balance = userData.money || 0;
            const buffer = await createBalanceCard(userData, targetID, balance, avatarImage);
